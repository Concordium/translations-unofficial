...
содержать :ref:`схему смарт-контрактов <contract-schema-ru>`.
...
.. seealso::
   ...
   См. :ref:`host-functions` для полной справки..
...
и для запуска в изолированных средах. Это полезно, потому что смарт-контракты будут
выполняться бекерами в сети, которые не обязательно доверяют коду.
...
.. todo::
   ...
   Добавить другие ограничения, например начальные разделы...

Блокчейн среда очень специфична в том смысле, что каждый узел должен иметь возможность
выполнять контракт аналогичным образом, используя точно такое же количество ресурсов.
...
Хотя в Wasm есть поддержка чисел с плавающей запятой, но смарт-контракты не могут их
...
Это ограничение применяется статически, а это означает, что смарт-контракты не могут содержать
типы с плавающей запятой и не могут содержать никаких инструкций, связанных со значениями
с плавающей запятой.
...
байт-кода и взимается как за проверку корректности модуля, так и за хранение в сети.
...
.. seealso::
   ...
   См. :ref:`contract-instances-ru` для получения дополнительной информации.
...
Поскольку модуль смарт-контракта может экспортировать функции для нескольких различных
смарт-контрактов, мы связываем эти функции с помощью схемы именования:

- ``init_<contract-name>``: Функция для инициализации смарт-контракта должна начинаться
  с ``init_`` и далее имя смарт-контракта. Контракт должен состоять только из буквенно-цифровых
...
.. note::
   ...
   процедурные макросы ``#[init(...)]`` и ``#[receive(...)]`` устанавливают правильную
   схему именования.
